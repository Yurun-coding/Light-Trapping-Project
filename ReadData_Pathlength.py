...
## Plotting
sns.histplot(total_pathlengths, bins=range(0, int(max(total_pathlengths)) + 101, 100), alpha=0.7, color='blue', edgecolor='black')

plt.title('Histogram of Pathlengths')
plt.xlabel('Total Pathlength (um)')
...
data = total_pathlengths

# Calculate histogram data
bin_edges = range(0, int(max(total_pathlengths)) + 101, 100)
...
excel_file_path = os.path.join(folder_path, 'histogram_data_10M_pathlength.xlsx')
